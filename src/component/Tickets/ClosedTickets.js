...
class ClosedTickets extends React.Component{
	...
	state = {
		...
		filterr: '',
		showFilter: false,
		ticketss: closedTickets,
		error: '',
	}

	onFilterChange = (e) => {
		const filter = e.target.value;
		this.setState(() => ({filter,showFilter:true,ticketss:closedTickets}));
	};
	...
	onChange = (e) => {
		...
		if(filterr !== ''){
			if(filter === 'Department'){
				const fticekts = closedTickets.filter(
			...
			this.setState(() => ({ticketss: closedTickets}));
		}
	};
